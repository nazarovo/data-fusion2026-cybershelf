...
19.02.2026
---------------------------
Для готовых продать бессмертную душу за готовый submit на 0,8358710262976741 есть <a href='https://disk.yandex.ru/d/9mr4HzwV-fhAIg'>ссылка</a>. Учет проданых душ ведут Яндекс и Мефистофель. 


25.02.2026 
---------------------------
На прошедшем вчера <s>пленуме партии была обозначена партийная (линия) полилиния</s> митапе было велено решать задачу как многометочную. Отклоняющихся могут <s>исключить из партии</s> дропнуть с соревнования. Совсем умным подменят метрику. 

---------------------------
PyBoost как Сбер. Ныне не пользуюсь и радуюсь.

---------------------------
Нужно ли учить отдельную модель на каждый класс? 

Это нерационально. Если многометочная модель дает по классу скор 0.99, то выигрыш от отдельной модели по классу не превысит (1-0.99)/41. (Скор по классу можно вычислить занулив в саммите другие классы) В качестве параметров модели можно указывать предикты мультимодели (Если помнить про переобучение).
